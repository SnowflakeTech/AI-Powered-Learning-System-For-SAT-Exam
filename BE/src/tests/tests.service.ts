...
export class TestsService {
  ...
  async canAccessTest(user: any, testId: number) {
    ...
    const test = await this.testModel.findByPk(testId, { attributes: ['id', 'isPublic', 'userId'] });
    if (!test) return false;

    if ((test as any).isPublic) return true;
    if ((test as any).userId && Number((test as any).userId) === Number(user.id)) return true;
  ...
  async removeTest(testId: number, deleteOrphans = false) {
    ...
    await sequelize.transaction(async (t: any) => {
      ...
      await this.feedbackModel.update({ testId: null }, { where: { testId }, transaction: t });
      ...
      if (deleteOrphans && questionIds.length > 0) {
        ...
        if (orphanIds.length > 0) {
          ...
          await this.choiceModel.destroy({ where: { questionId: { [Op.in]: orphanIds } }, transaction: t });
          await this.questionModel.destroy({ where: { id: { [Op.in]: orphanIds } }, transaction: t });
        }
      }
    });
